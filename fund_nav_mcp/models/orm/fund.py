...
from fund_nav_mcp.utils.enums import (
    AbnormalType, FundStatus, FundNavStatus, FundType, FundRegulatoryType,
    FundDataSource, PeriodType, FundManagementType, ShareClass,
)
...
class Fund(Base):
    ...
    manager_person: Mapped[Optional['FundManagerPerson']] = relationship('FundManagerPerson', back_populates='funds')
    nav_records: Mapped[List['FundNav']] = relationship(
        "FundNav", back_populates="fund")
    returns: Mapped[List['FundReturn']] = relationship(
        "FundReturn", back_populates="fund")
...
class FundNav(Base):
    ...
    fund_id: Mapped[Optional[int]] = mapped_column(Integer, comment='基金产品ID，关联fund表的ID')
    ...
    # 关系
    fund: Mapped[Optional['Fund']] = relationship("Fund", back_populates="nav_records")
...
class FundReturn(Base):
    ...
    # 关系
    fund: Mapped[Optional['Fund']] = relationship("Fund", back_populates="returns")
...
class FundHolding(Base):
    ...
    # 关系
    fund: Mapped[Optional['Fund']] = relationship("Fund", back_populates="holdings")
